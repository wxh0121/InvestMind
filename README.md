...
- 定投计划：通过 Vercel Cron 在交易日 15:30 自动执行加仓，支持按金额或数量定投，并按资产计价货币扣除对应现金。
...
- `vercel.json` 已配置 `GET /api/cron/dca`，Cron 表达式为 `30 7 * * *`。
- Vercel Cron 使用 UTC 时区，`07:30 UTC` 对应北京时间每天 `15:30`。
- Cron 会读取云端备份里的定投计划，刷新对应资产价格，按资产币种扣除现金；现金不足会写入定投失败状态。
- Vercel Hobby 计划每天只能触发一次 Cron，所以交易日定投固定为北京时间 `15:30`，不再由前端每分钟轮询；周六、周日会自动跳过并顺延到下一个交易日。
